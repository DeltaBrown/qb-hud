...
RegisterNetEvent('bj-hud:nitro')
AddEventHandler('bj-hud:nitro', function(percent)
  nitro = percent
end)

RegisterNetEvent('bj-hud:client:useSeatbelt')
AddEventHandler('bj-hud:client:useSeatbelt', function(status)
    seatbeltIsOn = status
    SendNUIMessage({action = "seatbelt", status = seatbeltIsOn})
end)

RegisterNetEvent('bj-hud:client:toggleui')
AddEventHandler('bj-hud:client:toggleui', function(show)
    if show then
		toghud = true
		if IsPedInAnyVehicle(GLOBAL_PED, false) then
			DisplayRadar(true)
	...
	while true do
		...
		if IsPedInAnyVehicle(GLOBAL_PED, false) then
			...
			if seatbeltIsOn or IsRadarEnabled() or not toghud then 
				waittime = 500
				TriggerEvent("bj-hud:client:useSeatbelt", false)
				if inVehicle then
					inVehicle = false
		...
		if GLOBAL_PEDINVEH then
			...
			if IsControlJustReleased(0, 29) and (has_value(vehiclesCars, GLOBAL_PEDVEHCLASS) == true and GLOBAL_PEDVEHCLASS ~= 8) then
				if not seatbeltIsOn then
					...
					BJCore.Functions.Notify('Seatbelt Buckled.', 'success', 10000)
					TriggerEvent("veh:seatbelt", true)
				else
					TriggerEvent('InteractSound_CL:PlayOnOne','carunbuckle',0.3)
					BJCore.Functions.Notify('Seatbelt Unbuckled.', 'error', 10000)
                ...
                if not harnessOn then
                    if IsPedInAnyVehicle(GetPlayerPed(-1)) and GetVehicleClass(GetVehiclePedIsIn(GetPlayerPed(-1))) ~= 8 and GetVehicleClass(GetVehiclePedIsIn(GetPlayerPed(-1))) ~= 13 and GetVehicleClass(GetVehiclePedIsIn(GetPlayerPed(-1))) ~= 14 then
                        if not seatbeltIsOn then
                            ...
                            TriggerEvent("bj-hud:client:useSeatbelt", true)
                        else
                    ...
                    BJCore.Functions.Progressbar("harness_equip", "Racing harness off..", 5000, false, true, {
...
RegisterNetEvent("bj-hud:client:ejection")
AddEventHandler("bj-hud:client:ejection",function(plate)
...
local handbrake = 0
RegisterNetEvent('bj-hud:client:resetHandbrake')
AddEventHandler('bj-hud:client:resetHandbrake', function()
        ...
        if currentVehicle ~= nil and currentVehicle ~= false and currentVehicle ~= 0 then
            ...
            if driverPed == GLOBAL_PED then
                if (GetVehicleHandbrake(currentVehicle) or (GetVehicleSteeringAngle(currentVehicle)) > 25.0 or (GetVehicleSteeringAngle(currentVehicle)) < -25.0) then
                    if handbrake == 0 then
                        handbrake = 100
                        TriggerEvent("bj-hud:client:resetHandbrake")
                    else
                        handbrake = 100
                    end
                end
                            ...
                            if not seatbeltIsOn and not IsThisModelABike(currentVehicle) then
                                --if math.random(math.ceil(lastFrameVehiclespeed)) > 68 then --I dont like this
                                    if not harnessOn then
                                        ...
                                        TriggerServerEvent('bj-hud:server:DoHarnessDamage', harnessHp, harnessData)
                                    end
                                --end
...
RegisterNetEvent('bj-hud:client:useHarness')
AddEventHandler('bj-hud:client:useHarness', function(ItemData)
    ...
    if inveh and not IsThisModelABike(GetEntityModel(GetVehiclePedIsIn(ped))) then
        if not harnessOn then
            BJCore.Functions.Progressbar("harness_equip", "Racing harness on..", 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
                ...
                ToggleHarness(true)
                TriggerServerEvent('bj-hud:server:equipHarness', ItemData)
            end)
...
function ToggleHarness(toggle)
    harnessOn = toggle
    TriggerEvent("bj-hud:client:useSeatbelt", true)
    TriggerEvent('bj-hud:client:toggleHarness', toggle)
end