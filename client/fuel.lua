...
AddEventHandler("RefuelCar",function()
    ...
    if curw == w then
        ...
        if DoesEntityExist(targetVehicle) then
            ...
            TriggerEvent('BJCore:Notify', "Refueled", "success")
        else
            TriggerEvent('BJCore:Notify', "No Target", "error")
        end
    else
        TriggerEvent('BJCore:Notify', "Need a Gas Can", "error")
    end
end)
...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    refillVehicle()
    BJCore.Functions.Progressbar("refueling", "Refueling", timer, false, true, {
...
AddEventHandler("CarFuelAlarm",function()
    if not alarmset then
        alarmset = true
        local i = 5
        TriggerEvent('BJCore:Notify', "Low fuel.", "error")
        while i > 0 do
            ...
            if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(GLOBAL_PED)) < 5.0 then
                ...
                if IsControlJustPressed(1, Controlkey["generalUse"][1]) then
                    if curFuel >= 100 then
                        ...
                        TriggerEvent('BJCore:Notify', "Your gas tank is full", "error")
                    else
                        costs = math.ceil(costs)
                        BJCore.Functions.TriggerCallback('BJCore:HasMoney', function(hasMoney)
                            if hasMoney == true then
                                TriggerEvent('RefuelCarServerReturn')
                                TriggerEvent('BJCore:Notify', "Refueling your car", "success")
                            else
                                TriggerEvent('BJCore:Notify', "You dont have that amount", "error")
                            end
                        end, costs)
                    end
                end
