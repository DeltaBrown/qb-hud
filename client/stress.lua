...
RegisterNetEvent('bj-hud:client:UpdateStress')
AddEventHandler('bj-hud:client:UpdateStress', function(newStress)
    stress = newStress
end)
...
function GetShakeIntensity(stresslevel)
    local retval = 0.05
    for k, v in pairs(BJHud.Intensity["shake"]) do
...
function GetEffectInterval(stresslevel)
    local retval = 60000
    for k, v in pairs(BJHud.EffectInterval) do
        if stresslevel >= v.min and stresslevel < v.max then
            retval = v.timeout
            break
        end
...
Citizen.CreateThread(function()
    while true do
        ...
        elseif stress >= BJHud.MinimumStress then
            local ShakeIntensity = GetShakeIntensity(stress)
            ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', ShakeIntensity)
